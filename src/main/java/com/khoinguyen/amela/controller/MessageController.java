...
public class MessageController {
    ...
    public String viewMessagesAdmin(Model model, @ModelAttribute PagingDtoRequest pagingDtoRequest) {
        ...
        session.setAttribute(
                "url",
                "/messages?pageIndex=" + pagingDtoRequest.getPageIndex() + "&text=" + pagingDtoRequest.getText());
        return "layout/messages/message_list";
    }

    @GetMapping("/detail/{id}")
    @PreAuthorize("hasAnyAuthority('ADMIN', 'USER')")
    public String viewMessageDetail(Model model, @PathVariable long id) {
        // check permission
        if (!permissionResources.checkPermission(id)) {
            return "redirect:/notFound";
        }
        List<MessageScheduleDtoResponse> topMessages = messageScheduleService.getTopMessagesScheduleForUser(10L, id);
    ...
    @PreAuthorize("hasAuthority('ADMIN')")
    public String viewCreateMessages(Model model, @PathVariable(required = false) Long messageId) {
    ...
    public String createMessages(
            @Valid @ModelAttribute("message") MessageScheduleDtoRequest request, BindingResult result, Model model) {
        model.addAttribute("users", optionalValidator.findAllUser());
        // check validate
    ...
    public String viewUpdateMessages(Model model, @PathVariable Long id) {
        ...
        if (!model.containsAttribute("message")) {
            MessageScheduleUpdateResponse messageScheduleDtoResponse =
                    messageScheduleService.getByMessageScheduleId(id, "id");
            model.addAttribute("message", messageScheduleDtoResponse);

            // check time publish at in the past
            if (checkPublishAtInThePast(messageScheduleDtoResponse.getPublishAt())) return "redirect:/forbidden";
        }
    ...
    public String updateMessages(
        ...
        // check time publish at in the past
        if (checkPublishAtInThePast(request.getPublishAt())) return "redirect:/forbidden";
        ...
        if (!errors.isEmpty()) {
            redirectAttributes.addFlashAttribute(
                    "message", MessageScheduleMapper.toMessageScheduleUpdateResponse(request));
